...
class MessageContext:
    ...
    def brutally_add_message(self, role: str, text: str, images: list = None):
        """
        Добавляет сообщение в контекст, игнорируя режимы добавления.
...
class ChatGPTAgent:
    ...
    def response_from_chat_GPT(self, user_message: str, images: list = None) -> str:
        """
        ...
        assistant_response = self.__call_openai_api(trimmed_messages)
    ...
    def response_from_chat_GPT_with_chain_of_reasoning(self, analysis_depth: int, user_message: str, images: list = None, preserve_user_messages_post_analysis: bool = True) -> str:
        """
...
""")
        ...
        # print(f"Цепочка рассуждений. Поставленные задачи:\n{assistant_response}\n")
...
""")
    ...
    def brutal_response_from_chat_GPT(self, user_message: str, images: list = None) -> str:
        """
        ...
        temp_message = self.context.brutally_add_message("user", user_message, images)
    ...
    def __call_openai_api(self, messages: list, response_format: ) -> str or None:
        ...
        while count_of_attempts < max_attempts:
            if count_of_attempts > 0:
                ...
                print(f"Повторная попытка будет произведена через {t} сек.")
                time.sleep(t)
            try:
                ...
                print(f"Ошибка в __call_openai_api: {e}")
                count_of_attempts += 1
    ...
    def __trim_context(self, messages: list, max_total_tokens: int) -> list:
        ...
        # Проверка, что после удаления повторяющихся системных сообщений все еще не вмещается в лимит
        ...
        # Возвращаем обрезанный контекст
        return messages
